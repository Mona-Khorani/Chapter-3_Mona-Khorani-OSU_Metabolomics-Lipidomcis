...
png(file="pngnew106.tiff", width=7500, height=7073,res = 340)
...
ggplot(data=m, aes(x=log2FC, y=neglogp, col=m$fcthre, label=labelmet) )+
  ...
  guides(color = guide_legend(override.aes = list(size = 6.5)))+xlim(-1.5, 1.5)+
  theme_minimal() +
  ...
  labs(title="Volcano plot for comparision of Tg2576 vs WT", x="Log2(Fold Change)",y="-Log10(pvalue)")
